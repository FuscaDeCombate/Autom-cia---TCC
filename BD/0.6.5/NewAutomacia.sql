...
-- PROCEDURE: DESATIVAR FUNCIONÁRIO
...
BEGIN
	...
	BEGIN TRY
		...
		ELSE
		BEGIN
			-- Retornar erro específico baseado na validação que falhou
			IF (@Empresa_Existe = 0) 
	...
	END TRY
	BEGIN CATCH
		-- Fechar chave simétrica em caso de erro
		IF EXISTS (SELECT 1 FROM sys.openkeys WHERE key_name = 'EnK_Registro_C')
			CLOSE SYMMETRIC KEY EnK_Registro_C;
		SELECT 'Informações Inválidas' As 'Retorno_Desativa_Funcionario';
	END CATCH
END
GO

-- PROCEDURE: REATIVAR FUNCIONÁRIO
...
BEGIN
	...
	BEGIN CATCH
		...
		IF EXISTS (SELECT 1 FROM sys.openkeys WHERE key_name = 'EnK_Registro_C')
			CLOSE SYMMETRIC KEY EnK_Registro_C;
		SELECT 'Informações Inválidas' As 'Retorno_Reativa_Funcionario';
	END CATCH
END
GO

-- PROCEDURE: LISTAR FUNCIONÁRIOS DA EMPRESA
...
BEGIN
	...
	BEGIN CATCH
		...
		IF EXISTS (SELECT 1 FROM sys.openkeys WHERE key_name = 'EnK_Registro_C')
			CLOSE SYMMETRIC KEY EnK_Registro_C;
		SELECT 'Informações Inválidas' As 'Retorno_Lista_Funcionarios';
	END CATCH
END

GO
-- PROCEDURE: DESATIVAR PACIENTE
CREATE PROCEDURE Desativa_Paciente(
	@CPF_Paciente VARCHAR(11),
	@Senha_Paciente VARCHAR(32)
) AS
BEGIN
	DECLARE
		@SenhaArmazenada VARCHAR(32),
	...
	BEGIN TRY
		...
		BEGIN
			...
			IF @SenhaArmazenada = @Senha_Paciente
				SET @Senha_Valida = 1;
		END
	...
	BEGIN CATCH
		...
		IF EXISTS (SELECT 1 FROM sys.openkeys WHERE key_name = 'EnK_Rec_P')
			CLOSE SYMMETRIC KEY EnK_Rec_P;
		SELECT 'Informações Inválidas' As 'Retorno_Desativa_Paciente';
	END CATCH
END
GO

-- PROCEDURE: REATIVAR PACIENTE
...
BEGIN
	...
	BEGIN TRY
		...
		BEGIN
			...
			SELECT 'Paciente reativado com sucesso. Recupere a senha se necessário.' As 'Retorno_Reativa_Paciente';
		END
		ELSE
		BEGIN
			-- Retornar erro específico baseado na validação que falhou
	...
	END TRY
	BEGIN CATCH
		SELECT 'Informações Inválidas' As 'Retorno_Reativa_Paciente';
	END CATCH
END

GO
-- PROCEDURE: ALTERAR SENHA FUNCIONÁRIO
CREATE PROCEDURE Alt_Funcionario(
	@ID_Funcionario INT,
	@Nova_Senha VARCHAR(32),
	@CNPJ_Contratante VARCHAR(20),
	@Senha_Contratante VARCHAR(32)
) AS
BEGIN
	...
	BEGIN TRY
		...
		BEGIN
			...
			UPDATE Funcionario 
			SET Func_Rec = ENCRYPTBYKEY(KEY_GUID('EnK_Func_Rec'), @Nova_Senha)
			WHERE Funcionar_Rec = @ID_Funcionario;
		...
		BEGIN
			...
			ELSE IF (@Nova_Senha_Valida = 0) 
				SELECT 'Nova senha inválida (mínimo 6 caracteres)' As 'Retorno_Alt_Funcionario';
		END

	END TRY
	BEGIN CATCH
		-- Fechar chaves simétricas em caso de erro
		IF EXISTS (SELECT 1 FROM sys.openkeys WHERE key_name = 'EnK_Registro_C')
			CLOSE SYMMETRIC KEY EnK_Registro_C;
		IF EXISTS (SELECT 1 FROM sys.openkeys WHERE key_name = 'EnK_Func_Rec')
			CLOSE SYMMETRIC KEY EnK_Func_Rec;
		SELECT 'Informações Inválidas' As 'Retorno_Alt_Funcionario';
	END CATCH
END
...
--Teste 15: Listar Funcionarios (Desativados)
...
EXEC Reativa_Paciente '54856098802', 'algumEmail@gmail.com', 'Alan2';
GO
