...
Create Table Funcionario (
	ID_Funcionario BIGINT Identity Unique Not null,
...
Create Table Receita (
	ID_Receita BIGINT Identity Not null,
	...
	@Fonee Varchar(16)
		...
		Else 
			...
			--Insere as informações caso passe por todas as verificações
				Insert Into Paciente(CPF, Senha_Paciente, Email, Nome_Paciente, Nome_Social, Fone) 
					Values (@CPFT, @SenhaT, @EmailT, @NomeT, @Nome_SocialT, @Fonee);
...
Create Procedure Envia_Mensagem_P (
	...
	@Mensagem Varchar(255)
		...
		Begin Try
			...
			IF (@ID_Mensageiro_R = 1) and (@ID_Receptor_R) = 1 Insert Into Mensagem (CPF, ID_Funcionario, Mensagem, Hora_Envio) Values (@ID_Mensageiro, @ID_Receptor, @Mensagem, @Hora);
			Else If (@ID_Mensageiro_R = 0) Select 'CPF Inválido' As 'Mensagem_Retorno_P';
			Else If (@ID_Receptor_R = 0) Select 'Funcionário Inválido' As 'Mensagem_Retorno_P';
...
Create Procedure Envia_Mensagem_F (
	...
	@Mensagem Varchar(255)
		...
		Begin Try
			...
			IF (@ID_Mensageiro_R = 1) and (@ID_Receptor_R) = 1 Insert Into Mensagem (CPF, ID_Funcionario, Mensagem, Hora_Envio) Values (@ID_Receptor, @ID_Mensageiro, @Mensagem, @Hora);	
...
Create Procedure Mostra_Chat(
		...
		Declare
			@CPF_M_Chat_R Bit,
			@ID_Funcionario_M_Chat_R Bit
		Begin Try
			Set @CPF_M_Chat_R = (Select Count(CPF) From Paciente);
			Set @ID_Funcionario_M_Chat_R = (Select Count(ID_Funcionario) From Funcionario);
			IF (@CPF_M_Chat_R = 1) and (@ID_Funcionario_M_Chat_R = 1) Select Mensagem From Mensagem Order By Hora_Envio ASC;
			Else IF (@CPF_M_Chat_R = 0) Select 'CPF Inválido' As 'Retorno_Mostra_Chat';
			Else IF (@ID_Funcionario_M_Chat_R = 0) Select 'Funcionário Inválido' As 'Retorno_Mostra_Chat';
...
Create Procedure Alt_Senha_P(
		...
		Begin Try
			...
			Set @Alt_CPF_R = (Select Count(CPF) From Paciente);
			If (Len(@Alt_Senha) >= 6) Set @Alt_Senha_R = 1;
			If (@Alt_CPF_R = 1) and (@Alt_Senha_R = 1) Update Paciente Set Senha_Paciente = @T_Alt_Senha Where CPF = @Alt_CPF;
			Else If (@Alt_CPF_R = 0) Select 'CPF Inválido' As 'Alt_Senha_Retorno';
			Else IF (@Alt_Senha_R = 0) Select 'Senha Inválida' As 'Alt_Senha_Retorno';
...
Create Trigger Validade_Receita On Receita After Update As
	Begin
		...
		Begin Try
			Declare
				@diff int;
			Set @diff = (Select Limite_Baixas From Receita Where ID_Receita = (Select ID_Receita From inserted)) - (Select Baixas From Receita Where ID_Receita = (Select ID_Receita From inserted));
			If (@diff <= 0) Update Receita Set Valido = 0 Where ID_Receita = (Select ID_Receita From inserted);
			Select 'Uma receita sofreu baixa' As 'Retorno_Trigger_Validade_R';
		End Try
		Begin Catch
			Select 'Erro' As 'Retorno_Trigger_Validade_R';
		End Catch
	End
Go
--Procedure que deve ser rodada regularmente sempre que possivel
Create Procedure Atualiza_Receita As 
	Begin Try
		Declare @quant BigInt, @i BigInt;
		Set @quant = (Select Count(ID_Receita) From Receita);
		While @i <= @quant
			Begin
				If (DATEDIFF(Day, GETDATE(), (Select Data_Validade From Receita))) < 0 Update Receita Set Valido = 0 Where ID_Receita = @i;
				Set @i = @i+1;
			End
	End Try
	Begin Catch
		Select 'Erro' As 'Retorno_Atualiza_Receita';
	End Catch
Go
