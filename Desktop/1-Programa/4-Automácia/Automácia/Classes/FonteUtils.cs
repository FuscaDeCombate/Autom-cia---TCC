...
public static class FonteUtils
{
    ...
    private static Dictionary<string, FontFamily> fontesCarregadas = new Dictionary<string, FontFamily>();
    private static List<PrivateFontCollection> colecoes = new List<PrivateFontCollection>(); // evita GC
    ...
    //---------------------------------------------Métodos sobre a Fonte dos Controles--------------------------------------------
    //----------Método Para Adicionar a Fonte como Recurso Externo----------
    private static extern IntPtr AddFontMemResourceEx(
        IntPtr pbFont, uint cbFont, IntPtr pdv, [In] ref uint pcFonts);


    //----------Método para Aplicar a Fonte em um Controle----------
