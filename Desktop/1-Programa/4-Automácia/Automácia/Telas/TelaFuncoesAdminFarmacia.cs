...
namespace Automácia
{
    public partial class TelaFuncoesAdminFarmacia : Form
    {
        ...
        //----------Função de Mostrar Baixas----------
        // Fundo do Label de Receitas Aprovadas
        ...
        //----------Função de Perfil do Funcionarios----------
            ...
            try
            {
                if (PnlPerfilFuncionario.Visible == true)
                {
                    ...
                    else
                    {
                        ...
                        TxtVerSenha.Focus();
                        SystemSounds.Beep.Play(); // cria o "beep" padrão
                        MessageBox.Show("Informações inválidas!", "Alteração não Sucedida!!!");
                    }
                }
                else
                {
            ...
            try
            {
                ...
                if (houveAlteracao)
                {
                    ...
                    for (int i = 0; i < dtFuncionarios.Rows.Count; i++)
                    {
                        ...
                        {
                            case "Funcionário Farmácia":
                                tipoFuncionario = "F";
                                break;
                            case "Administrador Farmácia":
                                tipoFuncionario = "A";
                                break;
                            default:
                                tipoFuncionario = "I";
                        ...
                        botao.Click += (s, f) =>
                        {
                            ...
                            GerarReceitasAprovadas(int.Parse(botao.Name));
                        };
        ...
        //-----------Gerar dinamicamente as Receitas Aprovadas----------
        private void GerarReceitasAprovadas(int idFuncionario)
        {
            try
            {
                // Limpa os labels antigos, se houver
                PnlReceitasAprovadas.Controls.Clear();

                // Declaração da Classe que pega do Banco e do DataTable necessário
                ClsBanco objBanco = new ClsBanco();
                DataTable dtReceitas = objBanco.ConsultaBaixas(idFuncionario);

                // Verificar se tem Receitas Aprovadas
                if (dtReceitas.Columns.Count <= 1)
                {
                    return;
                }

                // Para ajustar o Comprimento dos Labels de acordo com o scroll
                ...
                for (int i = 0; i < dtReceitas.Rows.Count; i++)
                {
                    ...
                    DateTime dataReceita = DateTime.Parse(dtReceitas.Rows[i]["Data_Baixa"].ToString());
                    ...
                    label.Text = $"Receita {idReceita}  {data}  {horario}";
        ...
        private void CadastrarFuncionario()
        {
            try
            {
                ...
                if (retornoLogin.Columns.Count > 1)
                {
                    ...
                    // Verificar se o Cadastro foi Válido
